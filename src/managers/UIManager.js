...
class UIManager {
    constructor(viewer, eventBus) {
        ...
        this.connectionPanel = new ConnectionPanel(this.viewer);
        ...
        if (this.eventBus) {
            ...
            this.eventBus.subscribe('element:updated', (data) => {
                // If the updated element is currently selected, refresh the edit interface
                ...
                if (selected.length === 1 && selected[0] === data.elementId) {
                    ...
                    if (freshElementData) {
                        // Use LeftSidebar for element editing instead of center EditPanel
                        if (window.leftSidebar && window.leftSidebar.currentContent) {
                            let uiConfig = null;
                            ...
                            // Refresh the edit mode with updated data
                            window.leftSidebar.showEditPanel(freshElementData, uiConfig);
                        }
                    }
                }
    ...
    _createBaseUI() {
        ...
        // connectionPanel is positioned absolutely, not in right column
        ...
        container.appendChild(this.connectionPanel.panel);
    ...
    _createTopToolbar() {
        ...
        creators.forEach(CreatorClass => {
            ...
            btn.className = 'btn';
            // Open sidebar with parameters instead of direct creation
            btn.addEventListener('click', () => {
                if (window.leftSidebar) {
                    window.leftSidebar.showOperation(`add-${meta.type}`);
                } else {
                    // Fallback to direct creation if sidebar not available
                    this.viewer.creationManager.startCreation(meta.type);
                }
            });
        ...
        connectionsBtn.id = 'connections-btn';
        connectionsBtn.textContent = '🔗 Connections';
        connectionsBtn.className = 'btn';
        connectionsBtn.addEventListener('click', () => this.connectionPanel.toggle());
        topToolbar.appendChild(connectionsBtn);

        // Add Hole operation button
        const holeBtn = document.createElement('button');
        holeBtn.id = 'add-hole-btn';
        holeBtn.textContent = '⚫ Add Hole';
        holeBtn.className = 'btn';
        holeBtn.addEventListener('click', () => {
            if (window.leftSidebar) {
                window.leftSidebar.showOperation('add-hole');
            }
        });
        topToolbar.appendChild(holeBtn);

        // Sidebar collapse/expand button removed - users can use the sidebar's own toggle button
    ...
    _createSnapToolbar() {
        ...
        snapToolbar.innerHTML = `
            ...
            <button id="snap-connections" class="snap-button" title="Snap to connection points">🔗</button>
            <div class="snap-separator"></div>
            <button id="connection-mode" class="snap-button connection-mode" title="Enable connection mode">🔗</button>`;
        return snapToolbar;
    }
    ...
    setupUI() {
        ...
        // this.populateAllProfileDropdowns(); // Disabled - using LeftSidebar instead
    }
    
    populateAllProfileDropdowns() {
        const allProfiles = this.viewer.profiles.getProfileNames();

        // this.creationPanel.populateProfileDropdowns(allProfiles); // Disabled - using LeftSidebar instead
    ...
    onSelectionChanged(selectedElements, selectionType = 'element') {
        ...
        if (selectedElements.length === 1) {
            if (selectionType === 'gridline') {
                // Handle grid line selection - for now just show status, no editing
                const gridLineId = selectedElements[0];
                // const gridLineData = this.getGridLineData(gridLineId);
                // if (gridLineData) {
                //     this.editPanel.show(gridLineData, null); // Disabled - no grid editing in sidebar yet
                // }
                this.updateStatusBar(`Grid line selected: ${gridLineId}`);
            } else {
                // Handle element selection - redirect to LeftSidebar
                ...
                if (element) {
                    ...
                    if (window.leftSidebar) {
                        ...
                        window.leftSidebar.showEditPanel(element, uiConfig);
                    } else {
                        console.warn('❌ LeftSidebar not available - element editing disabled');
                    }
                }
                this.updateStatusBar(null);
            }
        } else {
            // Hide edit interface when no single element is selected
            // this.editPanel.hide(); // Disabled - using LeftSidebar instead
            if (window.leftSidebar && window.leftSidebar.isEditMode) {
                // Exit edit mode in sidebar when selection changes
                window.leftSidebar.showWelcomePanel();
            }
    ...
    showCreationPanel(creatorClass) {
        // Show creation panel in LeftSidebar
        if (window.leftSidebar) {
            window.leftSidebar.showCreationPanel(creatorClass);
        } else {
